My [last post](/blog/from-autocomplete-to-autonomy) was about how AI changed the way I build software, how I went from writing code to directing development. That post made a lot of claims about productivity and workflow shifts. This one is a concrete example.
...
The constraints: cheap (ideally free), fast (no waiting seconds for a response), and simple (no infrastructure beyond what Rails already provides). No separate AI service, no Redis, no Postgres, no vector database. Just the same SQLite + Solid Queue stack the rest of the site runs on.
...
Five lines. RubyLLM is provider-agnostic through OpenAI-compatible APIs, so pointing it at Groq is just a base URL change. The `Chat` model uses `acts_as_chat` and the `Message` model uses `acts_as_message`. RubyLLM handles persistence, conversation history, and tool call tracking automatically.
...
[Groq](https://groq.com) runs inference on custom hardware (LPUs) that's optimized for speed. Responses start arriving in milliseconds, not seconds. For a chat agent on a personal site, that speed difference is everything. Nobody wants to wait three seconds for an answer to "What does Henrique specialize in?"
...
Most AI projects reach for complexity too early. I deliberately avoided it.

I didn't build a RAG pipeline. Retrieval-Augmented Generation is great when you have thousands of documents. I have a personal site with a few sections and a handful of blog posts. The entire site's content fits comfortably in a system prompt. I literally load `public/llms.txt` and call it a day.

I didn't set up a vector database either. No Pinecone, no pgvector, no Qdrant. SQLite `LIKE` queries are fine when you're searching a few blog posts. Would this scale to thousands of documents? No. Do I have thousands of documents? Also no.

And I didn't fine-tune anything. A well-written system prompt plus three tool classes gives the agent all the personality and knowledge it needs. Fine-tuning is for when you need the model to behave in ways that prompting can't achieve.

Start with the simplest thing that works. You can always add complexity later, but removing it is much harder.
...
The model decides when to use tools. If someone asks "What does Henrique do?", it answers from the system prompt. If someone asks "What has he written about AI?", it calls `SearchBlogPostsTool`. This happens automatically based on the tool descriptions.
...
The streaming implementation uses entirely standard Rails primitives. The agent streams responses in real-time, characters appear as the model generates them, just like ChatGPT. All ActionCable, Turbo Streams, and a background job.
...
No React, no custom WebSocket code. Hotwire handles all of it. The message form submits via Turbo, the job streams chunks back, and the UI updates in real-time. A few years ago this would have been a full SPA project.

## Cost and model flexibility

Running an AI agent on a personal site means every conversation costs something. With Groq's free tier and `gpt-oss-120b`, that cost is effectively zero. I'm not paying per token, just staying within rate limits.
...
What matters architecturally is that switching models is a one-line change. If I wanted to upgrade to GPT-4o:
...
Claude Code executed the implementation. I directed what to build and why. That division of labor is exactly what I described in the first post. This agent is a concrete example of it working.
...
Go try it at [hencf.org/chat](/chat). If you're a Rails developer thinking about adding AI features to your app, the barrier is lower than you'd expect. RubyLLM, Groq's free tier, and standard Hotwire primitives get you surprisingly far.
